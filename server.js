...
// Expose SMTP email (only the email username, not host/password) for active sessions
app.get('/api/smtp-info', async (req, res) => {
  ...
  try {
    const session = await dbGet(
      'SELECT s.* FROM sessions s WHERE s.token = ? AND s.logout_time IS NULL',
      [token]
    );
    if (!session) {
      return res.status(401).json({ error: 'Invalid or expired session.' });
    }
    return res.status(200).json({ email: process.env.SMTP_USER || null });
  } catch (err) {
    console.error('SMTP info route error:', err);
